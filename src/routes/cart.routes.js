...
const cartManager = new CartManager('src/models/cart.json', 'src/models/products.json')
...
routerCart.get('/', async(req, res) => {
    ...
    if(carts.length === 0){
        res.status(404).send("Carrito no hallado")
    } else {
...
routerCart.delete('/:cid', async(req, res) => {
    ...
    if(succesfullDelete) res.status(200).send("Producto eliminado del carrito correctamente")

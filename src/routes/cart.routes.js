...
cartRouter.put('/:cid/products/:pid', async (req, res) => {
  ...
  try {
    ...
    if (searchCart) {
      const productIndex = searchCart.products.findIndex(product => product.id_prod.toString() === pid)
      ...
      if (productIndex !== -1) {
        ...
        res.status(200).send({ result: 'OK', message: 'Cantidad del producto', cart: updatedCart })
      } else {
    ...
    res.status(400).send({ error: `Error al actualizar producto del carrito: ${error}` })
  }
})
